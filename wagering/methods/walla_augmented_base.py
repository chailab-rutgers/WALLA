"""
Shared augmented WALLA wagering implementation.

This module provides a reusable base class for V1/V2 augmented variants with:
...
from wagering.utils.tensor_helpers import build_mlp, l2_normalize_rows, sigmoid_row_normalize
from wagering.utils.walla_mechanism import extract_mechanism_components


class WallaAugmentedBase(WageringMethod):
    """Reusable augmented base for WALLA wagering variants."""
    ...
    def _build_head(self) -> nn.Module:
        return build_mlp(self.common_hidden_dim, self.hidden_layers, 1).to(self.device)
    ...
    def _project_and_route(
        ...
        for i in range(self.num_models):
            ...
            if self.normalize_hidden_states:
                hs_tensor = l2_normalize_rows(hs_tensor)
    ...
    def _normalize_wagers(self, raw_wagers_tensor: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        return sigmoid_row_normalize(raw_wagers_tensor, self.temperature)
    ...
    def _extract_components(
        ...
        return extract_mechanism_components(
            sigmoid_wagers, model_logits_tensor, gold_label_tensor
        )
