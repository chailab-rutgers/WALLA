"""
Stacked generalization: MLP meta-learner over concatenated LLM hidden states.
"""
...
from wagering.utils.checkpoint_loading import load_optimizer_state
from wagering.utils.tensor_helpers import build_mlp, project_hidden_states_list


class StackedGeneralization(WageringMethod):
    """
    Wagering method that uses an MLP meta-learner over concatenated hidden states.
    ...
    def __init__(self, num_models: int, config: Optional[Dict[str, Any]] = None):
        """
        Initialize stacked generalization.
        ...
        self.router = build_mlp(input_dim, self.hidden_layers, num_models).to(self.device)
    ...
    def compute_wagers(
        ...
        **kwargs
        ...
        Projects each model's states, concatenates them, then routes through the meta-learner.
        ...
        projected_batch_list = project_hidden_states_list(
            hidden_states_list,
            self.model_projections,
            self.hidden_dim,
            self.device,
            normalize=self.normalize_hidden_states,
            training=self._training,
            optimizer=self.optimizer if self._training else None,
        )

        # Concatenate all projected states
        batch_hidden_states_cat = torch.cat(projected_batch_list, dim=1)  # [batch_size, num_models * common_hidden_dim]

        # Step 3: Route through meta-learner
    ...
    def update(
        ...
        **kwargs
        ...
        if (self._cached_wagers is not None and
            ...
            raise ValueError(
                "StackedGeneralization.update() requires cached wagers from compute_wagers(). "
